...
Pixels are emitted as RGB565: rrrrrggg gggbbbbb — the same packing as
display_color() in lib/display/display.c. (The panel's filter is physically
B-G-R, but the driver leaves the MADCTL BGR bit set so the swap happens in
hardware and the data we ship stays ordinary RGB565.)
...
def rgb565(r, g, b):
    """Match display_color() in lib/display/display.c."""
    return ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | (b >> 3)


def to_pixels(rgba, count, bg):
    """RGBA -> list of RGB565, compositing transparent pixels onto bg."""
    px = []
...
def main():
    ...
    out.append(f"// {w}x{h}, RGB565 (same packing as display_color()).")
